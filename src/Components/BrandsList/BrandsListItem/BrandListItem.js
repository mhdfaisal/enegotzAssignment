...
const renderBrandListIem = ({ make, logo, additionalInfo = null }) => {
  return (
    <div className={`my-4 ${styles.brandsListItemContainer}`}>
      <div className={styles.brandsListImageContainer}>
        <div>
          <img src={logo} className={styles.brandListImage} alt="logo" />
        </div>
      </div>
      <div className="align-self-center">
        <Link to={`/list/${make}`}>
          <h4>{make.toUpperCase()}</h4>
        </Link>
